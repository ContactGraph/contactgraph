...
def test_function_name_from_arq_job_without_unpickling() -> None:
    raw = serialize_job("enrich_org", ("arg",), {}, None, 123)

    assert _function_name_from_job(raw) == "enrich_org"


def test_function_name_from_job_does_not_execute_malicious_pickle(tmp_path) -> None:
    ...
    raw = pickle.dumps({"t": None, "f": Payload(), "a": (), "k": {}, "et": 123})
