...
from contactsafe_server.db.models import OAuthCredential, Source, UserPersonObservation
...
class GoogleCalendarImportService:
    def __init__(self, db: AsyncSession, encryptor: TokenEncryptor, calendar_client: CalendarApiClient) -> None:
        self._db = db
    ...
    async def run_sync(self, source_id: uuid.UUID) -> None:
        ...
        try:
            cred = await self._get_credential_for_source(source)
            if cred is None:
                raise ValueError("No valid Google OAuth credentials for calendar source")
            access = self._encryptor.decrypt(cred.access_token_encrypted)
            refresh = self._encryptor.decrypt(cred.refresh_token_encrypted)
            access, refreshed = await self._calendar.get_valid_access_token(access, refresh, cred.token_expires_at)
    ...
    async def _fetch_and_ingest(self, *, access_token: str, source: Source) -> None:
        try:
            ...
            if exc.response.status_code == 410 and source.sync_token is not None:
                ...
                await self._db.flush()
                page = await self._calendar.list_recent_participants(access_token, sync_token=None, max_results=250)
            else:
                raise
        ...
        for participant in page.participants:
            ...
            stmt = pg_insert(UserPersonObservation).values(
                ...
                set_={"last_observed_at": now, "relationship_types": ["google_calendar"]},
            )
