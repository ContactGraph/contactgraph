...
class PersonDiscoveryService:
    ...
    async def _search_activity(
        ...
        *,
        ...
        if self._settings.exa_api_key:
            try:
                hits = await self._exa.search_person_activity(name=name, org_hint=org_hint)
                if hits:
        ...
        if self._settings.tavily_api_key:
            try:
                hits = await self._tavily.search_person_activity(name=name, org_hint=org_hint)
                if hits:
        ...
        if self._settings.serper_api_key:
            try:
                hits = await self._serper.search_person_activity(name=name, org_hint=org_hint)
                if hits:
